...
# 🛑 YANGI TOKENINGIZ JOYLASHDI
BOT_TOKEN = "8300065405:AAF06AxjTEny7zfQxVEFk38Cw3hGCIijxb8"
bot = telebot.TeleBot(BOT_TOKEN, parse_mode="Markdown")

# Xavfsiz operativ xotira xazinasi (Railway uchun mukammal)
DB = {
    "target_usernames": [],
    "users": {},
    "media_filters": {},
...
def send_welcome(message):
    bot.reply_to(message, "🚀 Yangi bot noldan muvaffaqiyatli ishga tushdi! Meni guruhga qo'shib, admin huquqini bering.")
...
def tag_all_members(message):
    ...
    if not DB["active_members"]:
        bot.reply_to(message, "⚠️ Guruhda hali faol a'zolar yo'q.")
        return
    msg_text = message.text.split(maxsplit=1)[1] if len(message.text.split()) > 1 else "Diqqat hamkorlar!"
...
def monitor_group_messages(message):
    ...
    if not is_admin(message.chat.id, user_id):
        ...
        if not has_access:
            ...
            bot.send_message(message.chat.id, f"⚠️ Guruhda yozish uchun odam qo'shishingiz kerak! Siz qo'shgan odamlar soni: {count}")

if __name__ == '__main__':
    bot.remove_webhook()
    time.sleep(1)
    print("Railway Polling muvaffaqiyatli boshlandi...")
