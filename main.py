...
# JSON fayldan ma'lumotlarni yuklash yoki yangi ochish
def load_db():
    if os.path.exists(DB_FILE):
        try:
            with open(DB_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
                # all_members ro'yxatini qaytadan set (to'plam) qilib olamiz
...
def save_db():
    try:
        data_to_save = {
            ...
            "all_members": list(DB["all_members"]) # set obyektini jsonga yozib bo'lmaydi, list qilamiz
        }
...
# 📢 OMMAVIY CHAQIRUV (/call)
@bot.message_handler(commands=['call'])
def tag_all_call(message):
    # Telegramdan kelgan IDlar bo'yicha foydalanuvchilarni yig'ish
    unique_users = []
    seen_ids = set()
    
    # Guruh adminlarini Telegram serveridan yangi yuklab olish
    try:
    ...
    # JSON bazada yig'ilgan eski a'zolarni ham qo'shamiz
    for member_id in DB["all_members"]:
        if member_id not in seen_ids:
            seen_ids.add(member_id)
            try:
                # Bot foydalanuvchini IDsi orqali tanib olishga urinadi
    ...
    if not unique_users:
        bot.reply_to(message, "⚠️ Chaqirish uchun a'zolar ro'yxati hozircha bo'sh. Guruhga bittagina xabar yozing va qaytadan /call bering.")
        return
...
# 🛡 GURUHDA ODAM YOZSA BAZAGA YIG'ISH
...
def collect_active_users(message):
    if message.chat.type != "private" and not message.from_user.is_bot:
        if message.from_user.id not in DB["all_members"]:
            ...
            save_db() # JSONga yozamiz

# 🔄 YANGI ODAM QO'SHILGANDA YOKI O'ZI KIRGANDA
@bot.message_handler(content_types=['new_chat_members'])
def handle_new_members(message):
    adder_id = message.from_user.id
    adder_str = str(adder_id)

    # Yangi kirgan odamni bazaga qo'shish
    ...
    for member in message.new_chat_members:
        ...
        if member.username:
            ...
            if user_tag in TARGET_USERNAMES:
                ...
                save_db() # Qo'shgan odamlar sonini JSONga saqlaymiz
...
if __name__ == '__main__':
    bot.remove_webhook()
    time.sleep(1)
    print("JSON ma'lumotlar bazali bot ishga tushdi...")
